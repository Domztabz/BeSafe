package ke.co.visualdiagnoser.besafe.ui.onboarding.fragment.personal
...
import ke.co.visualdiagnoser.besafe.ui.onboarding.fragment.enternumber.EnterNumberFragment
...
class PersonalDetailsFragment : PagerChildFragment() {
    ...
    override val navigationIcon: Int = ke.co.visualdiagnoser.besafe.R.drawable.ic_up
...
//        personal_details_post_code.addTextChangedListener(postCodeTextWatcher)
        ...
        personal_details_county?.setOnClickListener {
            showRadioButtonDialogCounties()
        }


    }

    private fun showRadioButtonDialogCounties() {
        val stringList = resources.getStringArray(R.array.counties) // here is list
        val view: View = requireActivity().getLayoutInflater().inflate(R.layout.radiobutton_dialog, null)
        //TODO: Replace radio buttons with checkboxes for more flexible filters
        val rg = view.findViewById<View>(R.id.radio_group) as RadioGroup
        for (i in stringList.indices) {
            val chosenCategory = stringList[i]
            val rb = RadioButton(requireActivity()) // dynamically creating RadioButton and adding to RadioGroup.
            rb.text = chosenCategory
            rb.setOnClickListener { personal_details_county.setText(chosenCategory) }
            rg.addView(rb)
        }
        val alertDialogBuilder = AlertDialog.Builder(requireActivity())
        alertDialogBuilder.setView(view)
        alertDialogBuilder.setTitle("Select Your County")
...
//        personal_details_county.removeTextChangedListener(postCodeTextWatcher)
    ...
    override fun getUploadButtonLayout(): UploadButtonLayout = UploadButtonLayout.ContinueLayout(R.string.personal_details_button) {
        presenter.saveInfos(personal_details_name.text.toString(), personal_details_county.text.toString(), getMidAgeToSend())
    }

    override fun updateButtonState() {
        if (presenter.validateInputsForButtonUpdate(personal_details_name.text.toString(), personal_details_county.text.toString(), getMidAgeToSend())) {
