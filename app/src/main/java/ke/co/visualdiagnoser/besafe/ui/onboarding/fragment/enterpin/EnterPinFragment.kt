...
import ke.co.visualdiagnoser.besafe.extensions.toHyperlink
...
class EnterPinFragment : PagerChildFragment() {
    ...
    companion object {
        ...
        const val ENTER_CLEANSED_PIN_PHONE_NUMBER = "cleansed_phone_number"
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            val censoredPhoneNumber = it.getString(ENTER_CLEANSED_PIN_PHONE_NUMBER)
            verificationId = it.getString(ENTER_VERIFICATION_ID)

            val token = if (it.containsKey(ENTER_TOKEN)) it.get(ENTER_TOKEN) as PhoneAuthProvider.ForceResendingToken else null
            ...
            enter_pin_headline.text = resources.getString(R.string.enter_pin_headline, resources.getString(R.string.enter_number_prefix), censoredPhoneNumber)
...
//            SystemUtils.hideSoftKeyboard(view)
    ...
    private fun validateOtp() {
        verificationId?.let {
            presenter.validateOTP(it, requireView().pin.value)
        }
    }
